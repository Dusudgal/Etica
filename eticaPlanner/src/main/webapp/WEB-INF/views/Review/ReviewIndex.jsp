...
<body>
    ...
    <script>
        ...
        async function findtouristSpot(event){
            ...
            try {
                const response = await fetch('/Planner/TourApiSearch', {
                    ...
                    body: JSON.stringify({ keyword: encodedString })
                });
                if (!response.ok) {
                ...
                if (Array.isArray(jsondata) && jsondata.length > 0) {
                        ...
                        tourList += `
                            <li>
                                ...
                                <form action="/Review/ReviewGeneration" method="GET">
                                    ...
                                    <button type="submit" class="button" onclick="return confirmLogin();">리뷰 생성</button>
                                </form>
                    ...
                    errmessage.appendChild(document.createTextNode(`검색하신 ${event}에 대한 정보가 없습니다.`));
        ...
        function confirmLogin() {
            const isConfirmed = confirm("로그인 후 이용 가능한 서비스입니다. 로그인 하시겠습니까?");
            if (isConfirmed) {
                window.location.href = "/user/sign-in-view"; // 로그인 페이지 URL로 변경
            }
